...
class Sitemap {
    ...
    public function add_language_variants_to_entry( $url, $type, $obj ) {
        ...
        // Yoast may provide entries generated by another multilingual plugin.
        // Do not turn an explicitly localized URL such as /fr/page into a
        // nested Localize URL such as /zh-tw/fr/page. The configured source
        // language remains eligible so explicit /en/ URLs are still handled.
        if ( $this->has_explicit_non_source_language( $url['loc'] ) ) {
            return $url;
        }
    ...
    /**
     * Check whether a URL starts with an explicit non-source language code.
     *
     * @param string $url The sitemap URL.
     * @return bool
     */
    private function has_explicit_non_source_language( $url ) {
        ...
        $first_path_segment = isset( $parts[0] ) ? strtolower( $parts[0] ) : '';

        if ( ! preg_match( '/^[a-z]{2}(?:-[a-z]{2})?$/i', $first_path_segment ) ) {
            return false;
        }

        return strtolower( Common::get_source_language() ) !== $first_path_segment;
    }
