...
?>
<?php if (get_the_term_list(get_the_ID(), 'event_type')) :
	$nameterms = wp_get_post_terms(get_the_ID(), 'event_type');
	$formatted_terms = array();
	foreach ($nameterms as $term) {
		$format_slug = $term->slug;
		$format_name = $term->name;
		$term_link = add_query_arg('event_type', $format_slug, $current_url);
		if (str_contains($site_url, '/ja-jp/')) {
...
<?php if (get_field('event_content_type') == 'external') : ?>
	...
	<div class="card-container">
		...
		<div class="card-content-container">
			...
			<div class="card-bottom">
				<?php if (get_field('external_url')) : ?>
					<a aria-label="<?php the_title_attribute() ?>" class="card-link" title="<?php the_title_attribute() ?>" href="<?php echo $href; ?>" target="<?php echo $target; ?>">
					<?php else : ?>
						<a aria-label="<?php the_title_attribute() ?>" class="card-link" title="<?php the_title_attribute() ?>" href="<?php echo $href; ?>" target="<?php echo $target; ?>">
						<?php endif; ?>
						<?php if ($cta_label) {
