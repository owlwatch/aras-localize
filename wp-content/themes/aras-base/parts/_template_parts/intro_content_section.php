<?php if (have_rows('intro_content_sections')) : ?>
	<?php while (have_rows('intro_content_sections')) : the_row(); ?>
		...
		<?php if (get_sub_field('content_type') == 'text') : ?>
			...
			<?php if (have_rows('text_block')) : ?>
				<?php while (have_rows('text_block')) : the_row(); ?>
					...
					<div class="text-block wysiwyg-content formsideblock">
		...
		<?php elseif (get_sub_field('content_type') == 'image') : ?>
			...
			<?php if (have_rows('image_block')) : ?>
				<?php while (have_rows('image_block')) : the_row(); ?>
					...
					if (!empty($image)) : ?>
						<div class="image-block text-center formsideblock">
			...
			<?php if (have_rows('video_block')) : ?>
				<?php while (have_rows('video_block')) : the_row(); ?>
					...
					<?php else : ?>
						<?php $poster = (''); ?>
					<?php endif; ?>
					<div class="video-block formsideblock">
		...
		<?php elseif (get_sub_field('content_type') == 'split') : ?>

			<?php if (have_rows('side-by-side_content_block')) : ?>
				<?php while (have_rows('side-by-side_content_block')) : the_row(); ?>

					<?php if (get_sub_field('vertical_alignment') == 'top') : ?>
						<?php $valign = 'align-top'; ?>
					<?php elseif (get_sub_field('vertical_alignment') == 'bottom') : ?>
						<?php $valign = 'align-bottom'; ?>
					<?php else : ?>
						<?php $valign = 'align-middle'; ?>
					<?php endif; ?>

					<div class="split-text-block wysiwyg-content formsideblock <?= "$valign" ?>">
						<?php if (get_sub_field('left_content')) : ?>
							<div class="split-text-half">
								<?php echo get_sub_field('left_content'); ?>
							</div>
						<?php endif; ?>
