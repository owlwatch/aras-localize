...
function update_academic_users_from_file()
{
	...
	foreach ($academic_users_data['records'] as $record) {
		...
		$academic_user_id = $record['fields']['Id']['value'];
		$Aras_Website__c = $record['fields']['Aras_Website__c']['value'];
		...
		if ($Aras_Website__c == true) {
			$existing_posts = get_posts([
				...
				'meta_key' => 'aras_academic_user_id',
				'meta_value' => $academic_user_id,
				'post_status' => 'any',
				'numberposts' => 1
			]);
			...
			if ($existing_post) {
				...
				update_post_meta( $existing_post->ID, 'aras_academic_user_id', $academic_user_id);
			} else {
				//New partner means new post
				$post_args = array(
					'post_title' => $Name,
				...
				update_post_meta( $new_post_id, 'aras_academic_user_id', $academic_user_id);
			}
		}
	}
	...
	if (have_rows('flexible_content', $post_id)) :
		while (have_rows('flexible_content', $post_id)) : the_row();
			if (get_row_layout() == 'split_content_section' || get_row_layout() == 'full_width_form_section') :
				...
				if (get_sub_field('create_salesforce_campaign', $post_id)) {
					// Check if the field 'salesforce_campaign' exists and is empty
				...
				if (have_rows('right_content', $post_id)) :
					while (have_rows('right_content', $post_id)) : the_row();
						if (have_rows('form_block', $post_id)) :
							while (have_rows('form_block', $post_id)) : the_row();
								if (get_sub_field('create_salesforce_campaign', $post_id)) {
									// Check if the field 'salesforce_campaign' exists and is empty
									$salesforce_campaign = get_sub_field('salesforce_campaign', $post_id);
									if (!$salesforce_campaign) {
				...
				if (have_rows('left_content', $post_id)) :
					while (have_rows('left_content', $post_id)) : the_row();
						if (have_rows('form_block', $post_id)) :
							while (have_rows('form_block', $post_id)) : the_row();
								if (get_sub_field('create_salesforce_campaign', $post_id)) {
									// Check if the field 'salesforce_campaign' exists and is empty
...
function create_new_salesforce_campaign($title_of_page, $url_of_page, $language, $format_resource_type)
{
	...
	$body_params = array(
		...
		'IsActive' => 'true',
		'RecordTypeId' => '0125f000000zEQWAA2',
