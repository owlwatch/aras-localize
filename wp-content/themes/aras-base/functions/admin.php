...
function joints_rss_dashboard_widget() {
	...
	if ($limit == 0) echo '<div>' . __( 'The RSS Feed is either empty or unavailable.', 'aras' ) . '</div>';   // fallback message
	else foreach ($items as $item) { ?>

	<h4 style="margin-bottom: 0;">
		<a href="<?php echo $item->get_permalink(); ?>" title="<?php echo mysql2date(__('j F Y @ g:i a', 'aras'), $item->get_date('Y-m-d H:i:s')); ?>" target="_blank">
...
function joints_custom_dashboard_widgets() {
	wp_add_dashboard_widget('joints_rss_dashboard_widget', __('Custom RSS Feed (Customize in admin.php)', 'aras'), 'joints_rss_dashboard_widget');
	/*
...
function joints_custom_admin_footer() {
	_e('<span id="footer-thankyou">Developed by <a href="#" target="_blank">Your Site Name</a></span>.', 'aras');
}
