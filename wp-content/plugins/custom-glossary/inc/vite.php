...
const VITE_HOST = 'https://localhost:5133';
...
function urlBase()
{
    static $base;
    if (!isset($base)) {
        $base = plugins_url('', __DIR__) . '/frontend/dist/';
    }
    return $base;
}
...
{
    $content = file_get_contents(dirname(__DIR__) . '/frontend/dist/manifest.json');
...
function importsUrls(string $entry): array
{
    ...
    if (!empty($manifest[$entry]['imports'])) {
        foreach ($manifest[$entry]['imports'] as $imports) {
            $urls[] = urlBase() . $manifest[$imports]['file'];
        }
    }
    return $urls;
}
